...
**NÄCHSTER SCHRITT (Apply, netz-los/reversibel):** `scp deploy/zone-avv-vendor.yaml → pz2:/var/lib/vz/snippets/` → `qm set 159 --cicustom vendor=local:snippets/zone-avv-vendor.yaml` → `qm set 159 --cipassword <bootstrap>` → `cloud-init clean` → `qm start`/reboot → Serial-Verify (`qm terminal 159`): cloud-init done, 18 Dateien gelandet (Pfad/Perm/sha-Spotcheck), Drop-in-Symlink, `systemctl start zone-netns-setup` → netns-Topologie (br-zone+5 Seat-netns+IPv6-off). Dann ttyd (P8). STUFE 2 (Egress) nach Netzi-VLAN50 + Provider-Pin.
