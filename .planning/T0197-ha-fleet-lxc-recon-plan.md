...
### 2d. Baustein 3 — Cross-Host-Messaging (Netzwerk, mit Netzi/orchestrator-network)
...
- **Netzi-Position (read-only gemessen 2026-06-12):** **(a) Reachability schon offen, KEINE UDM-Änderung nötig** — 42er→20er liegen beide in der LAN-Zone (terminales ACCEPT), WG(192.168.16.0/24)→20er via VPN-Zone (terminales ACCEPT). Nur die Host-Binds sind zu ändern. **(b) Fail-closed-Scoping = host-level nftables AUF DEM LXC** (Netzi-Empfehlung, objektiv besser als UDM-Regel): die LAN-Zone umfasst **7 Subnets** (20/42/6/10/30/99/168) → 0.0.0.0-Bind würde die auth-losen :7890/:7899 für ALLE VLANs (inkl. IoT) öffnen. Allowlist: src 192.168.42.0/24 + 192.168.20.0/24 + 192.168.16.0/24 → :7890/:7899, Rest DROP. **HA-Argument (entscheidend):** host-interne nftables wandern beim pz1↔pz3-Failover MIT dem LXC; eine UDM-Regel hinge an der LXC-IP = zweiter Pflege-Punkt. WAN-Seite eh default-deny v4+v6 + keine PF → kein Public-Leak.
- **Aufteilung:** nftables-Regelset auf dem LXC = mein Build (Infra-Hoheit); Netzi liefert das exakte Regelset + Mess-Oracle (nc-Matrix aus 42er/20er/WG/Fremd-VLAN), sobald LXC-IP feststeht + Hub-GO. Optional UDM-ACL als 2. Schicht falls Schnüffi defense-in-depth will (sein R22 auf den Bind-Change entscheidet). Netzi bis dahin HOLD.
