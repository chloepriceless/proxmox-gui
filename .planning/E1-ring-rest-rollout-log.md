...
3. **unpoller apt kaputt (Drittanbieter):** `pkg.unpoller.com` löst auf die HAUS-WAN-IP 87.139.158.187 auf → Cert-Verify-Fail → `update_cache` bricht. Workaround: sudo direkt installiert + `install_qemu_guest_agent=false`. **Ursache (Netzi live gemessen, KORREKTUR meiner ersten Annahme „Resolver-Rewrite"):** kein Resolver-Rewrite — die Domain ist extern tot (kein A-Record), aber eine **öffentliche Wildcard `*.bikini.bottom.zone → Haus-WAN-IP`** + DHCP-Search-Domain `bikini.bottom.zone` führt dazu, dass libc das Suffix an den fehlgeschlagenen Lookup hängt (`pkg.unpoller.com.bikini.bottom.zone`) → Wildcard-Match → Haus-IP hinter TLS-Fail. Fix-Owner: Kuma (Repo-File raus, ERLEDIGT — unpoller ist eh tot: Binary statt deb, Service inactive, keine Metriken → **LXC148-Dekommission-Kandidat**, an Christin/Hub gebündelt) + Netzi (Wildcard-A-Record in öffentlicher Zone löschen, Christin-gated; zeigt zudem auf die dynamische PPPoE-IP = Eigen-Risiko bei IP-Wechsel).
