...
**Plans**: 7 plans
- [ ] 02-01-connector-extension-PLAN.md — Extend PVEConnector with cache + pybreaker + 6 new read/write methods; PVEConnectorRegistry.get_for_team() + per-cluster health probe; busy_timeout PRAGMA confirmation
- [ ] 02-02-audit-schema-writer-PLAN.md — 0003_phase2 migration (per-cluster quota columns + audit_log filter indices); audit writer/reader/CSV stream; GET /api/v1/audit + GET /api/v1/audit/export.csv (UTF-8 BOM, 50000-row cap, CSV-injection escape); source_ip helper
- [ ] 02-03-inventory-backend-PLAN.md — Inventory module (routes/service/schemas/RRD/access); GET /me/inventory + /clusters/{id}/inventory + per-VM detail + RRD; PUT tags + PUT notes; per-team privsep token + pool-match RBAC; audit-on-mutation with token-scrubbing
- [ ] 02-04-quotas-backend-PLAN.md — Quota CRUD + admission (BEGIN IMMEDIATE) + /me/quotas aggregator + /quotas/preview; D-12 lower-anyway via allow_over flag; audit-on-quota-change
