...
- **✅ PRE-BUILD-GATE DURCHGEFÜHRT (2026-06-21) → Verdikt `.planning/T0244-REFUTE-VERDICT.md`.** Doc-aware Kombi-Refute (Input `T0244-REFUTE-INPUT.md` @ e9b3333, eingebetteter Inhalt = bwrap-blind-v1-Fix) mit **zwei konvergenten Cross-Lab-Lensen** (Claude-Subagent + Schnüffis GPT-codex; Qwen-Gateway down). **Verdikt: NOT-BUILD-READY (Egress)** — beide Lensen konvergieren auf den Kern-Blocker B1 (Nur-Broker-Enforcement nur BEHAUPTET, nicht seat-perspektivisch-negativ-getestet; Oracle misst falsche Ebene). **Foundation-ohne-Egress von beiden freigegeben.** → **NEUER Gate vor egress-fähigem Bau = die Pre-Build-Artefakt-Liste** (Verdikt §3): Requirements zu konkreten, seat-negativ-testbaren Artefakten machen (Schnüffi: LLM-Broker-RPC-Spec + Detektor-positive-Allowlist; ich: netns/nftables-SSOT + Seat-Negativ-Oracle + Cap-Drop + Boot-fail-closed + ip_forward-Beweis + IPv6-Drop + vzdump-Policy + Pseudonym-Map-Substrat; Netzi: VLAN-networkgroup-Artefakt + Allowlist-Final). KEIN egress-fähiger Bau bis Artefakte vorliegen + negativ-getestet + Konsens.
