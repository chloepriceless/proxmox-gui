...
2. **🔴 Scan ALLER physischen QUARANTINE-Objekte, nicht der Ref-Diffs (R4/B1+B2 + R5/B1 — Mechanismus festgenagelt):** ein Rev-Walk `<old>..<new>` / `rev-list --objects` erfasst **nicht** alle eingegangenen Objekte — ein **unreachable Klartext-Blob im gepushten Pack** + der **signed-push-cert-Blob (`GIT_PUSH_CERT`)** landen nach Accept auf Disk, würden aber von einem Ref-Diff-Scan übersehen (G1 deckt „unreachable/pack" explizit ab; §1.7-`gc` ist reaktiv, nicht präventiv). **🔴 `git cat-file --batch-all-objects` ist FALSCH (R5/B1):** es gibt laut git-Doku Repo **+ Alternates** aus → over-included den ganzen bestehenden Main-Store, beweist NICHT „physisch in Quarantine".
   **🔴 Branch-Signal „objekttragend" = AUSSCHLIESSLICH rohe physische Datei-Präsenz, UNABHÄNGIG vom OID-Parse (R7/BLOCKER — gegen zirkulären fail-open):** `raw_presence` := es existiert mind. eine rohe Datei in `$GIT_QUARANTINE_PATH/[0-9a-f][0-9a-f]/*` (loose) ODER `$GIT_QUARANTINE_PATH/pack/*` (pack). **`raw_presence` wird NIE aus `count(PHYS_OIDS)>0` abgeleitet** — sonst klassifiziert ein still brechender Parse (Glob/show-index liefert 0 trotz vorhandener Objekte) einen objekttragenden Push als „Ref-only" → Objekte migrieren ungescannt auf Disk = fail-open (die Vacuity-Klasse aus R4–R6). Regel: **`raw_presence==true` → PHYS_OIDS bilden; `raw_presence==true && (count(PHYS_OIDS)==0 ODER nicht-parsebar)` → REJECT** (Enumeration kaputt, nicht „nichts zu tun"). `raw_presence==false` → reine Ref-Op (§1.4 Konditional-Punkt 3).
   → **Exakte physische Quarantine-OID-Menge** (nur wenn `raw_presence`):
...
**🔴 NON-VACUITY als KONDITIONALE Invariante (R4/HIGH-1 + R5/B2 + R6/BLOCKER — Spec-Widerspruch behoben):** ein Hook, der einen **leeren** Objektsatz scannt, PASST sonst alles vacuously durch. **Die alte Form (`git cat-file -e <new_oid>` auf Ref-Tips) war selbst vacuous (R5/B2):** Ref-Tips lösen via Alternates auf Altobjekte auf. **ABER ein unbedingtes `count(PHYS_OIDS)>0 → REJECT` (R5-Form) ist FALSCH (R6/BLOCKER):** pre-receive läuft auch für **reine Ref-Operationen ohne Objekte** — Branch-Create auf ein *existierendes* Commit, Lightweight-Tag, **Ref-Delete** — das sind **legitime Null-Objekt-Pushes** (kein Pack), die das unbedingte count>0 fälschlich ablehnen würde (widerspräche §7.7(c)). → **Korrekt, fail-closed + konditional:**
   1. **Immer (jeder Push):** push-options-Check (`GIT_PUSH_OPTION_COUNT==0`), refname-Scan + ref-Typ-default-deny, ref-Tripel validieren.
   2. **Objekttragender Push (`raw_presence==true`, §1.4.2 — rohe Datei-Präsenz, NICHT aus count abgeleitet):** `PHYS_OIDS = loose ∪ pack-idx` bilden; **`raw_presence==true && (count(PHYS_OIDS)==0 ODER nicht-parsebar)` → REJECT** (Enumeration kaputt = fail-closed, NICHT als Ref-only fehlklassifizieren — R7/BLOCKER); sonst jedes Objekt scannen.
   3. **Reine Ref-Operation (`raw_presence==false`):** als Null-Objekt-Ref-Transaktion **zulassen** — objektseitig nichts zu scannen; refname-/push-option-/ref-Typ-Regeln gelten weiter.
   4. **Signed-Push:** `GIT_PUSH_CERT`-Blob MUSS in `PHYS_OIDS` enthalten + gescannt sein, sonst REJECT.
   **§1.4 und §7.7 sind damit wörtlich konsistent** und der Branch hängt am unabhängigen `raw_presence`-Signal, nicht zirkulär am OID-Parse. Harte Oracle-PASS-Bedingung (§7 Kriterium 7) mit **Positiv-Kontrolle**: injizierter unreachable-bad-blob + push-cert-Marker MÜSSEN in `PHYS_OIDS` auftauchen + REJECTED werden; **und „rohe Quarantine-Datei present, aber OID-Enumeration auf 0 gebrochen ⇒ REJECT" (R7)**.
...
**MED-Notizen (R6+R7, vor Live-Build verifizieren — keine Spec-Blocker):**
- **`receive.procReceiveRefs` absent (R6/R7-MED):** Forgejos/gits **proc-receive**-Mechanismus (für agit/Push-to-create) kann den normalen pre-receive-Objektfluss umgehen. Auf der Secrets-Instanz verifizieren, dass `receive.procReceiveRefs` **nicht gesetzt** ist (sonst landet Content über einen Pfad, den der Quarantine-Scan evtl. nicht sieht).
- **Positiv-Kontroll-Oracle = Test-Konstruktions-Detail (R6-MED):** der injizierte unreachable-bad-blob + push-cert für §7.7(b)/(d) wird über `git pack-objects` + `git receive-pack` gegen ein Test-Repo konstruiert → ins Build-Runbook.
- **Perf-Limits live kalibrieren (R7-MED):** `receive.maxInputSize` + Blob/Count-Limit + Hook-Timeout am echten LXC-160-Durchsatz einstellen (nicht raten), Oracle-oversized-Test mit dem kalibrierten Wert.
- **Hook-Wache real testen (R7-MED):** die Integritäts-Wache (§1.4-B4) nach echtem `forgejo admin regenerate hooks` + simuliertem Upgrade tatsächlich auslösen lassen (read-only-Schaltung beobachten), nicht nur spezifizieren.
...
7. **🔴 KONDITIONALE Non-Vacuity über die PHYSISCHE Quarantine-Menge (R4/HIGH-1 + R5/B2 + R6/BLOCKER + R7/BLOCKER, load-bearing):** beweisen, dass der Hook die physisch eingegangenen Objekte erfasst (nicht via Alternates die Altobjekte) — **ohne legitime Null-Objekt-Ref-Ops fälschlich abzulehnen UND ohne bei kaputter Enumeration fail-open zu gehen**. PASS-Bedingung, wörtlich konsistent mit §1.4-Konditional-Invariante: (a) **objekttragend (`raw_presence==true`):** `count(PHYS_OIDS)>0`, sonst REJECT — **NICHT** `cat-file -e` auf Ref-Tips (Alternates = vacuous); (b) **Positiv-Kontrolle:** injizierter unreachable-bad-blob (nur im Pack) + push-cert-Marker MÜSSEN in `PHYS_OIDS` erscheinen + REJECTED werden; (c) **reine Ref-Op (`raw_presence==false`: Branch-Create auf existierendes Commit / Lightweight-Tag / Ref-Delete):** wird **zugelassen**, aber refname-/push-option-/ref-Typ-Regeln greifen — Oracle: solcher Push NICHT fälschlich REJECTED, ein Secret-Refname dennoch geblockt; (d) **🔴 broken-enum (R7):** rohe Quarantine-Datei **present** (`raw_presence==true`), aber OID-Enumeration künstlich auf 0 gebrochen (z.B. Glob/show-index manipuliert) ⇒ **MUSS REJECT** (nicht als Ref-only durchwinken) — beweist, dass `raw_presence` unabhängig vom OID-Parse ist. Pflicht VOR allen anderen Negativtests.
...
- **Codex-Refute-Lens:** `codex-worker` ist in diesem Harness NICHT als Subagent-Typ verfügbar → fresh-context Claude-Lens (mein Refute, R26 — §9 gefoldet) + Schnüffis echte GPT-codex-Lens (§10 gefoldet, modell-divers). Etablierter T-0244-Pattern. **Stand: R1–R7 gefoldet (32 Befunde, Trend 12→7→2→5→4→1→1, holistisch clean). R7 = nur EIN Wortlaut-Fold (raw_presence als unabhängiges objekttragend-Signal gegen zirkulären fail-open) → behoben. codex erwartet R8 als Sign-off-Runde. Danach Gate nur noch LIVE-§7-Oracle (Canary auf LXC 160) + §8.**
...
**Nächster Gate-Schritt:** Schnüffis R6-Bestätigungs-Lens auf diese R5-Fassung. Sehr nah.

---

## 14. Refute-Round-6 (Schnüffi GPT-codex-Lens) — gefoldet (2026-06-22)
Verdikt: **NOT-READY, aber nur EIN BLOCKER — einen Fix vom Sign-off.** Trend R1:12→R2:7→R3:2→R4:5→R5:4→R6:1 (T-0244-R8→R9-Endspurt). codex bestätigte auf READY-Niveau: PHYS_OIDS-beide-Pfade sound, age-Whitelist `{recipient,enc}` mechanisch korrekt, Positiv-Kontroll-Oracle real konstruierbar — keine neue Klasse.
- **R6-BLOCKER (interner Spec-Widerspruch):** §1.4 sagte unbedingt `count(PHYS_OIDS)>0 sonst REJECT`, §7.7(c) erklärte Null-Objekt-Pushes (Branch-Create auf existierendes Commit / Lightweight-Tag / Ref-Delete) als legitim → §1.4 hätte genau das abgelehnt, was §7.7(c) als sicher deklariert (pre-receive läuft auch für reine Ref-Ops). → **konditionale Invariante** (Check NICHT droppen — das reintroduziert R5-B2-Vacuity): immer push-options+refname+ref-Tripel; objekttragend → `count>0` sonst REJECT + scannen; reine Ref-Op → zulassen (nichts objektseitig); Signed-Push → `GIT_PUSH_CERT` MUSS in `PHYS_OIDS`. §1.4 ↔ §7.7 wörtlich konsistent gemacht.
- **R6-MED (2, vor Live-Build):** `receive.procReceiveRefs` absent verifizieren (proc-receive-Bypass); Positiv-Kontroll-Oracle-Konstruktion (`pack-objects`+`receive-pack`) ins Runbook.

**Nächster Gate-Schritt:** Schnüffis R7-Sign-off-Lens auf diese R6-Fassung.

---

## 15. Refute-Round-7 (Schnüffi GPT-codex Sign-off-Lens) — gefoldet (2026-06-22)
Verdikt: **NOT-READY, aber holistisch CLEAN — EIN Wortlaut-Fold vom Sign-off.** Holistischer Pass über G1–G3/§1.4/§1.7/§2/§3/§7: KEIN weiterer Build-Blocker. Trend R1:12→R2:7→R3:2→R4:5→R5:4→R6:1→R7:1 (Wortlaut).
- **R7-BLOCKER (zirkulärer fail-open im Branch-Signal):** §1.4 verzweigte „objekttragend → count>0 / keine Objekte → zulassen", sagte aber nicht ausdrücklich, dass „objekttragend" UNABHÄNGIG vom OID-Parse bestimmt wird. Wäre das Signal als `count(PHYS_OIDS)>0` selbst implementiert → zirkulär: still brechende Enumeration (Parse=0 trotz Objekten) klassifiziert objekttragenden Push als „Ref-only" → ungescannt auf Disk = fail-open (die R4–R6-Vacuity-Klasse). → **`raw_presence` := rohe physische Datei-Präsenz** (`$GIT_QUARANTINE_PATH/[0-9a-f][0-9a-f]/*` ∪ `pack/*`), **NIE aus count abgeleitet**; `raw_presence==true && count==0/nicht-parsebar → REJECT`. §1.4.2 + §1.4-Konditional + §7.7(d) entsprechend normativ geschärft.
- **R7-MED (4, vor Live-Build):** `receive.procReceiveRefs` absent · Positiv-Kontroll-Konstruktion (`pack-objects`+`receive-pack`) ins Runbook · Perf-Limits live kalibrieren · Hook-Wache nach `regenerate hooks` real auslösen.

**Stand: ALLE Spec-Blocker zu** (R1–R7, 32 Befunde gefoldet, holistisch clean). codex erwartet **R8 als Sign-off-Runde** (nur normative Präzisierung, keine neue Klasse). Danach hängt das Gate nur noch am **LIVE-§7-Oracle** (Canary auf LXC 160: konditionale Non-Vacuity + Positiv-Kontrolle + Ref-only-Test + broken-enum-Test) + **§8** (op-connect weg von .240 + m-of-n Shamir gedrillt). Kein Klartext-Secret vorher.
