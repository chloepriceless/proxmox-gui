...
2. **🔴 Scan ALLER physischen QUARANTINE-Objekte, nicht der Ref-Diffs (R4/B1+B2 — tiefere Enforcement-Schicht):** ein Rev-Walk `<old>..<new>` / `rev-list --objects` erfasst **nicht** alle eingegangenen Objekte — ein **unreachable Klartext-Blob im gepushten Pack** + der **signed-push-cert-Blob (`GIT_PUSH_CERT`)** landen nach Accept auf Disk, würden aber von einem Ref-Diff-Scan übersehen (G1 deckt „unreachable/pack" explizit ab; §1.7-`gc` ist reaktiv, nicht präventiv). `rev-list --objects`-Pfade sind laut git-Doku nur **„Hints", nicht parsersicher**. → Der Hook enumeriert **ALLE physischen Objekte im `$GIT_QUARANTINE_PATH`** (eingehende loose+packed Objekte VOR Accept) und scannt **jedes nach Typ, NUL-sicher**:
...
**🔴 NON-VACUITY-Selbstcheck — der Hook MUSS beweisen, dass er die Objekte SIEHT (R4/HIGH-1, load-bearing):** ein Hook, der einen **leeren** Objektsatz scannt, PASST alles vacuously durch (false-PASS). → Der Hook prüft als ERSTES, fail-closed: **`$GIT_QUARANTINE_PATH` ist gesetzt UND nicht leer**, und **jede `<new_oid>` aus den ref-Tripeln ist mit `git cat-file -e <new_oid>` im Quarantine-Objektspeicher auflösbar** (= die Objekte sind wirklich da und scanbar). Schlägt das fehl (Quarantine unset/leer, OID nicht auflösbar) → **REJECT** (nicht „nichts zu tun → pass"). Diese Invariante ist auch eine harte Oracle-PASS-Bedingung (§7) — exakt die Non-Vacuity-Klasse aus T-0244 (Oracle darf nicht den eigenen Erwartungsraum bestätigen).
...
7. **🔴 Hook-SICHTBARKEITS-/Non-Vacuity-Invariante (R4/HIGH-1, load-bearing):** beweisen, dass der Hook die Quarantine-Objekte WIRKLICH sieht (sonst scannt er einen leeren Satz → PASST alles vacuously). PASS-Bedingung: (a) im Hook ist `$GIT_QUARANTINE_PATH` gesetzt+nicht-leer und jede `<new_oid>` per `git cat-file -e` auflösbar (sonst REJECT); (b) **Positiv-Kontrolle:** ein Canary-Push mit EINEM bekannt-bösen Objekt (Klartext-Blob) MUSS REJECTED werden — beweist, dass der Scanner einen nicht-leeren Satz prüft und greift; (c) ein „leerer"/0-Objekt-Push schmuggelt kein Secret durch. Diese Invariante ist Pflicht VOR allen anderen Negativtests (sie validiert, dass die Matrix nicht vacuously grün ist).

**Sign-off-Bedingung:** alle **7 Kriterien** grün (inkl. der erweiterten **18-Punkt-Negativ-Matrix a–r** UND der Non-Vacuity-Invariante) + Schnüffis GPT-codex-Bestätigungs-Lens findet keinen build-blockierenden Befund + Schnüffi-Sign-off + §8-Vorbedingungen erfüllt. Analog zum T-0244-Triple-Oracle-Gate.
...
**Nächster Gate-Schritt:** Schnüffis R5-Bestätigungs-Lens auf diese R4-Fassung. Trend weiter Richtung ready (tiefere Schicht abgeräumt, keine Regressionen). Kein Klartext-Secret vor grünem §7-Oracle inkl. Hook-Sichtbarkeits-Invariante + Sign-off + §8.
