...
**🔴 NON-VACUITY als KONDITIONALE Invariante (R4/HIGH-1 + R5/B2 + R6/BLOCKER — Spec-Widerspruch behoben):** ein Hook, der einen **leeren** Objektsatz scannt, PASST sonst alles vacuously durch. **Die alte Form (`git cat-file -e <new_oid>` auf Ref-Tips) war selbst vacuous (R5/B2):** Ref-Tips lösen via Alternates auf Altobjekte auf. **ABER ein unbedingtes `count(PHYS_OIDS)>0 → REJECT` (R5-Form) ist FALSCH (R6/BLOCKER):** pre-receive läuft auch für **reine Ref-Operationen ohne Objekte** — Branch-Create auf ein *existierendes* Commit, Lightweight-Tag, **Ref-Delete** — das sind **legitime Null-Objekt-Pushes** (kein Pack), die das unbedingte count>0 fälschlich ablehnen würde (widerspräche §7.7(c)). → **Korrekt, fail-closed + konditional:**
   ...
   2. **Objekttragender Push (Quarantine gesetzt+existent + enthält Objekte):** `PHYS_OIDS = loose ∪ pack-idx` bilden, **`count>0` sonst REJECT** (Enumeration muss greifen), jedes Objekt scannen.
   3. **Reine Ref-Operation (keine Quarantine / keine Objekte):** als Null-Objekt-Ref-Transaktion **zulassen** — objektseitig nichts zu scannen; refname-/push-option-/ref-Typ-Regeln gelten weiter.
   4. **Signed-Push:** `GIT_PUSH_CERT`-Blob MUSS in `PHYS_OIDS` enthalten + gescannt sein, sonst REJECT.
   **§1.4 und §7.7 sind damit wörtlich konsistent.** Harte Oracle-PASS-Bedingung (§7 Kriterium 7) mit **Positiv-Kontrolle gegen unabhängiges Signal**: ein injizierter unreachable-bad-blob + push-cert-Marker MÜSSEN in `PHYS_OIDS` auftauchen + REJECTED werden (objekttragender Fall) — beweist, dass die Enumeration die physische Menge wirklich erfasst.
...
**MED-Notizen (R6, vor Live-Build verifizieren — keine Spec-Blocker):**
- **`receive.procReceiveRefs` absent (R6-MED):** Forgejos/gits **proc-receive**-Mechanismus (für agit/Push-to-create) kann den normalen pre-receive-Objektfluss umgehen. Auf der Secrets-Instanz verifizieren, dass `receive.procReceiveRefs` **nicht gesetzt** ist (sonst landet Content über einen Pfad, den der Quarantine-Scan evtl. nicht sieht).
- **Positiv-Kontroll-Oracle = Test-Konstruktions-Detail (R6-MED):** der injizierte unreachable-bad-blob + push-cert für §7.7(b) wird über `git pack-objects` + `git receive-pack` gegen ein Test-Repo konstruiert → ins Build-Runbook, kein Spec-Blocker.
...
7. **🔴 KONDITIONALE Non-Vacuity über die PHYSISCHE Quarantine-Menge (R4/HIGH-1 + R5/B2 + R6/BLOCKER, load-bearing):** beweisen, dass der Hook die physisch eingegangenen Objekte erfasst (nicht via Alternates die Altobjekte) — **ohne legitime Null-Objekt-Ref-Ops fälschlich abzulehnen**. PASS-Bedingung, wörtlich konsistent mit §1.4-Konditional-Invariante: (a) **objekttragender Push:** `count(PHYS_OIDS)>0` (loose+pack, §1.4.2), sonst REJECT — **NICHT** `cat-file -e` auf Ref-Tips (Alternates = vacuous); (b) **Positiv-Kontrolle gegen unabhängiges Signal:** injizierter unreachable-bad-blob (nur im Pack) + push-cert-Marker MÜSSEN in `PHYS_OIDS` erscheinen + REJECTED werden; (c) **reine Ref-Op (Branch-Create auf existierendes Commit / Lightweight-Tag / Ref-Delete = legitimer Null-Objekt-Push):** wird **zugelassen** (objektseitig nichts zu scannen), aber refname-/push-option-/ref-Typ-Regeln greifen trotzdem — Oracle testet, dass ein solcher Push NICHT fälschlich REJECTED wird UND ein Secret-Refname dennoch abgelehnt wird. Pflicht VOR allen anderen Negativtests.
...
- **Codex-Refute-Lens:** `codex-worker` ist in diesem Harness NICHT als Subagent-Typ verfügbar → fresh-context Claude-Lens (mein Refute, R26 — §9 gefoldet) + Schnüffis echte GPT-codex-Lens (§10 gefoldet, modell-divers). Etablierter T-0244-Pattern. **Stand: R1–R6 gefoldet (31 Befunde, Trend 12→7→2→5→4→1, keine Regressionen). R6 = nur EIN Blocker (interner §1.4↔§7.7-Widerspruch, konditionale Non-Vacuity-Invariante) → behoben. codex erwartet R7 als Sign-off-Runde. Danach Gate nur noch LIVE-§7-Oracle (Canary auf LXC 160) + §8.**
...
**Stand: ALLE Spec-Blocker zu** (R1–R6, 31 Befunde gefoldet). codex erwartet **R7 als Sign-off-Runde**, wenn der R6-Fold §1.4↔§7.7 widerspruchsfrei macht. Danach hängt das Gate nur noch am **LIVE-§7-Oracle** (Canary auf LXC 160) + **§8** (op-connect weg von .240 + m-of-n Shamir). Kein Klartext-Secret vorher.
