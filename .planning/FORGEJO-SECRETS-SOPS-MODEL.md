...
2. **🔴 Klartext in Git-METADATEN, nicht nur Leaf-Werten (R2/B1 + R3-B1 generalisiert):** SOPS verschlüsselt **nur Werte**, lässt **Keys/Struktur/Pfade/Objekt-Header Klartext** (per Design). Ein Secret als Dateiname / Verzeichnis / Refname / Commit-Message / Author / YAML-KEY — **oder** in einem Objekt-Header (annotated-tag `tagger`, rohe Tag-Header, Commit-`encoding`, `gpgsig`/Signed-Commit-Payload, `mergetag` mit eingebettetem Tag-Objekt) — landet **Klartext**, ohne Blob/Leaf/Pfad/Ref/Msg/Author zu berühren. **Feld-für-Feld-Aufzählung = Whack-a-Mole** (T-0244-R7/R8-Lehre). → **Generisch: jedes NEUE Commit- UND Tag-Objekt komplett roh durch den Secret-Scanner** (`git cat-file -p <oid>`, normalisiert) — reine Object-IDs/Hashes ignorierbar, aber **Header, Signaturblöcke, `mergetag`, `tagger`, Pfade, Message, Identity MÜSSEN rein**; Treffer = REJECT. **Annotated Tags inkl. vollem Tag-Objekt = default-deny-Scan.** Plus der Datei-Scan (Tree-Pfadsegmente, Struktur-Keys, gitleaks) wie gehabt. **Tier-A-Empfehlung: SOPS-`--input-type binary`/dotenv** (ganzer Inhalt = EIN ENC-Blob, KEINE sichtbaren Keys) — eliminiert die Klartext-Key-Fläche *innerhalb* der Datei by construction; die Objekt-Header-Fläche bleibt über den Roh-Objekt-Scan abgedeckt.
...
4. **Nur native age, KEINE Zusatz-Backends — inkl. `key_groups` (R2/B2 + R3-B2):** SOPS kann **PGP/AWS-KMS/GCP-KMS/Azure-KV/hc_vault/SSH-age als ZUSÄTZLICHE Master-Keys** tragen → 2. Decrypt-Pfad, age-Liste bleibt scheinbar konform. → Hook erzwingt: `sops`-Stanza-Felder `pgp`/`kms`/`gcp_kms`/`azure_kv`/`hc_vault` UND **`key_groups`** (das Backends VERSCHACHTELN kann → „nur age" sonst nicht bewiesen) MÜSSEN konform sein. Regel: **`key_groups` entweder hart REJECT, ODER rekursiv validieren** (jede Gruppe nur `age1…`-Recipients, keine anderen Backends/SSH-age/unbekannten Felder). **Präzise leer-Semantik:** nur **absent ODER leere Liste** ok; **null / String / Objekt / unbekannter Typ → REJECT** (kein „truthy-Check"-Schlupfloch). Nur `age:` mit `age1…` erlaubt. Negativtest (top-level + nested key_groups) ins Oracle.
...
**Nächster Gate-Schritt:** Schnüffis R4-Lens auf diese R3-Fassung (sie recycelt evtl. Context — frische Session fährt R4 auf den R3-Fold-Commit). Trend klar Richtung ready. Kein Klartext-Secret vor grünem §7-Oracle + Sign-off + §8-Vorbedingungen.
