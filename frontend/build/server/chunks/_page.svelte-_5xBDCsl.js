import { m as escape_html, h as ensure_array_like, d as derived, c as store_get, f as bind_props, l as stringify, j as attr, o as spread_props, z as props_id, p as attributes, n as unsubscribe_stores, k as attr_class, q as clsx } from './renderer--hvGDOOw.js';
import { i as invalidateAll, g as goto } from './client-bCVElyx1.js';
import { p as page } from './stores-CCN9TkGO.js';
import { B as Button, c as cn } from './button-ntOmtgiY.js';
import { I as Input, c as createId, b as boxWith, m as mergeProps, d as attachRef, f as boolToEmptyStrOrUndef, a as createBitsAttrs, g as getDataOpenClosed, e as getDataTransitionAttrs, h as boolToStr } from './input-Buwd_f-b.js';
...
import { A as Alert_dialog, a as Alert_dialog_content, b as Alert_dialog_header, c as Alert_dialog_title, d as Alert_dialog_description, e as Alert_dialog_footer } from './alert-dialog-description-CQsrn2hX.js';
import { w as watch, C as Context, S as SPACE, E as ENTER } from './is-HJ-O5XFN.js';
import { o as on } from './root-DHp9To-z.js';
import { R as RovingFocusGroup, g as PresenceManager, j as afterTick } from './scroll-lock-CGoEUeDJ.js';
import { n as noop } from './noop-n4I-x7yK.js';
import { T as Table, a as Table_header, b as Table_row, d as Table_body, e as Table_cell, c as Table_head } from './table-row-Bxxa4BOD.js';
import { B as Badge } from './badge-yr7sVO8Y.js';
import { C as Chevron_down } from './chevron-down-C_ZM4_gT.js';
import { C as Chevron_up } from './chevron-up-BjMYcytG.js';
...
import { S as Shield_alert } from './shield-alert-C6UufUOa.js';
import { F as FilterChip } from './FilterChip-DePl9QOy.js';
import { P as Play, S as Square, a as Power, T as TagPill } from './TagPill-CfHuYA1s.js';
import { E as EmptyState, B as Boxes } from './EmptyState-Bb79vt8I.js';
import { a as api } from './client2-WJrlUD72.js';
import { b as formatAgo } from './format-Cqeoh9TR.js';
import { P as Plus } from './plus-CQ7kMicV.js';
import { L as List_checks, C as Chevron_right } from './chevron-right-x0q9RjrJ.js';
...
import { C as Circle_alert } from './circle-alert-DKfF-6ru.js';
import { C as Clock } from './clock-D1v6CFWO.js';
import { R as Rotate_cw } from './rotate-cw-C9Tc2DIl.js';
...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    const updatedLabel = derived(() => `Updated ${formatAgo((now - lastRefresh) / 1e3)}`);
    ...
    function inventoryCard($$renderer3, item) {
      $$renderer3.push(`<div class="relative rounded-md border border-border bg-card p-3 transition-colors hover:bg-muted/50"><div class="flex items-start justify-between gap-2"><div class="flex min-w-0 items-center gap-2">`);
      if (bulkMode) {
        $$renderer3.push("<!--[0-->");
        Checkbox($$renderer3, {
          checked: isSelected(item),
      ...
      if (bulkMode) {
        $$renderer3.push("<!--[0-->");
        $$renderer3.push(`<div class="truncate text-[14px] font-medium">${escape_html(vmName(item))}</div>`);
      } else {
        $$renderer3.push("<!--[-1-->");
        $$renderer3.push(`<a${attr("href", `/inventory/${item.cluster_id}/${item.vmid}`)} class="block truncate text-[14px] font-medium after:absolute after:inset-0 after:content-['']">${escape_html(vmName(item))}</a>`);
      }
      $$renderer3.push(`<!--]--> <div class="font-mono text-[13px] text-muted-foreground">${escape_html(item.vmid)} · ${escape_html(item.node)}</div></div></div> `);
      if (!bulkMode) {
        $$renderer3.push("<!--[0-->");
        $$renderer3.push(`<div class="relative z-10">`);
        rowMenu($$renderer3, item);
        $$renderer3.push(`<!----></div>`);
      } else {
        $$renderer3.push("<!--[-1-->");
      }
      $$renderer3.push(`<!--]--></div> <div class="mt-3 flex items-center justify-between gap-2"><span class="inline-flex items-center gap-1 text-[13px]">`);
      if (item.status === "running") {
        $$renderer3.push("<!--[0-->");
        Circle_play($$renderer3, { class: "size-4 text-success", "aria-hidden": "true" });
      } else if (item.status === "paused") {
        $$renderer3.push("<!--[1-->");
        Circle_pause($$renderer3, { class: "size-4 text-warning", "aria-hidden": "true" });
      } else if (item.status === "stopped") {
        $$renderer3.push("<!--[2-->");
        Circle_stop($$renderer3, { class: "size-4 text-muted-foreground", "aria-hidden": "true" });
      } else {
        $$renderer3.push("<!--[-1-->");
        Circle_alert($$renderer3, { class: "size-4 text-destructive", "aria-hidden": "true" });
      }
      $$renderer3.push(`<!--]--> <span>${escape_html(item.status)}</span> `);
      if (item.is_stale) {
        $$renderer3.push("<!--[0-->");
        Clock($$renderer3, { class: "size-3.5 text-warning", "aria-label": "Stale data" });
      } else {
        $$renderer3.push("<!--[-1-->");
      }
      $$renderer3.push(`<!--]--></span> `);
      if (item.tags.length > 0) {
        $$renderer3.push("<!--[0-->");
        $$renderer3.push(`<div class="flex flex-wrap justify-end gap-1"><!--[-->`);
        ...
        if (item.tags.length > 3) {
          ...
          Badge($$renderer3, {
            variant: "outline",
            children: ($$renderer4) => {
              $$renderer4.push(`<!---->+${escape_html(item.tags.length - 3)}`);
            },
            $$slots: { default: true }
          });
        } else {
    ...
    function resourceBlock($$renderer3, filtered) {
      $$renderer3.push(`<div class="hidden rounded-md border border-border md:block">`);
      if (Table) {
        $$renderer3.push("<!--[-->");
        Table($$renderer3, {
      ...
      $$renderer3.push(`</div> <div class="flex flex-col gap-3 md:hidden"><!--[-->`);
      const each_array_3 = ensure_array_like(filtered);
      for (let $$index_3 = 0, $$length = each_array_3.length; $$index_3 < $$length; $$index_3++) {
        let item = each_array_3[$$index_3];
        inventoryCard($$renderer3, item);
      }
      $$renderer3.push(`<!--]--></div>`);
    }
    let $$settled = true;
    let $$inner_renderer;
    function $$render_inner($$renderer3) {
      $$renderer3.push(`<header class="flex flex-row items-start justify-between gap-4 mb-6"><div class="flex flex-col gap-1"><h1 class="text-[28px] font-semibold tracking-tight">Inventory</h1> <p class="text-muted-foreground text-[14px]">Your VMs and LXCs across all clusters.</p> <p class="text-muted-foreground text-[13px]">Status changes can take up to ~20 s to appear here (Proxmox cluster
      cache + GUI cache).</p></div> `);
      Button($$renderer3, {
        href: "/create",
        class: "shrink-0",
        children: ($$renderer4) => {
      ...
      $$renderer3.push(` <span class="text-muted-foreground text-[13px] tabular-nums whitespace-nowrap" aria-live="polite">${escape_html(updatedLabel())}</span> `);
      Button($$renderer3, {
        variant: "outline",
        size: "sm",
        ...
        children: ($$renderer4) => {
          Refresh_cw($$renderer4, {
            class: `size-3.5 ${stringify(refreshing ? "animate-spin" : "")}`,
            "aria-hidden": "true"
          });
      ...
      if (filterActive()) {
        ...
        $$renderer3.push(`<!--]--> <!--[-->`);
        const each_array_4 = ensure_array_like(Array.from(statusFilter()));
        ...
        $$renderer3.push(`<!--]--> <!--[-->`);
        const each_array_5 = ensure_array_like(Array.from(tagFilter()));
      ...
      if (data.loadError) {
        ...
        if (filtered.length === 0) {
          ...
          $$renderer3.push("<!--[-1-->");
          resourceBlock($$renderer3, filtered);
        }
        $$renderer3.push(`<!--]-->`);
      } else {
        $$renderer3.push("<!--[-1-->");
        if (Accordion) {
          ...
          Accordion($$renderer3, {
            ...
            children: ($$renderer4) => {
              $$renderer4.push(`<!--[-->`);
              const each_array_6 = ensure_array_like(clusters());
                ...
                ClusterSection($$renderer4, {
                  ...
                  children: ($$renderer5) => {
                    if (filtered.length === 0) {
                      ...
                      $$renderer5.push("<!--[-1-->");
                      resourceBlock($$renderer5, filtered);
                    }
                    $$renderer5.push(`<!--]-->`);
                  }
                });
...
//# sourceMappingURL=_page.svelte-_5xBDCsl.js.map
